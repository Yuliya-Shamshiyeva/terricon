...
//Обобщения (Generics) в языке программирования Kotlin предоставляют средство для создания кода,
// который может работать с различными типами данных, обеспечивая при этом безопасность типов.
// Обобщения позволяют писать универсальный код, который не зависит от конкретных типов данных, с которыми он работает.
//
//Основные понятия:
//Параметризация типов (Type Parameters): Обобщения используют параметры типов для предоставления общего интерфейса
// или функциональности для различных типов данных.
// Параметры типов обозначаются в угловых скобках (angle brackets) после имени класса или функции.
//
//kotlin
//Copy code
//class Box<T>(val value: T)
//Использование обобщенных классов и функций:
// Обобщенные классы и функции могут принимать аргументы типа при создании экземпляров или вызове.
//
//kotlin
//Copy code
//val intBox = Box(42)    // Создание экземпляра обобщенного класса с параметром типа Int
//val stringBox = Box("Hello")  // Создание экземпляра обобщенного класса с параметром типа String
//
//fun <T> printItem(item: T) {
//    println(item)
//}
//
//printItem(10)     // Вызов обобщенной функции с аргументом типа Int
//printItem("World")  // Вызов обобщенной функции с аргументом типа String
//Безопасность типов (Type Safety):
// Обобщения предоставляют безопасность типов, позволяя компилятору проверить типы данных на этапе компиляции,
// что предотвращает ошибки связанные с типами данных на этапе выполнения.
//
//kotlin
//Copy code
//val value: String = intBox.value  // Ошибка компиляции: Type mismatch. Required: String, Found: Int
//Обобщенные функции с ограничениями (Bounds): В Kotlin можно использовать ограничения для ограничения типов данных,
// которые могут использоваться в параметрах типов.
//
//kotlin
//Copy code
...
//}
//
//val intSquare = square(5)   // Корректно, возвращает Int
//val doubleSquare = square(3.5)  // Корректно, возвращает Double
//Варианты типов (Type Variance): Kotlin поддерживает варианты типов (in, out),
// которые определяют, как обобщенные типы могут быть использованы в подтипах и супертипах.
//
//kotlin
//Copy code
...
//}
//Обобщения в Kotlin упрощают и обобщают код,
// делая его более гибким и безопасным с точки зрения типов данных.
// Они являются важным инструментом для написания гибких и многоразовых компонентов.